...
import org.jage.algorithms.jssp.jp.JobShopEvaluator;
 ...
 * Time: 13:51
 */
public class RechenbergSwapMutation extends RechenbergAbstractStochasticMutate<Integer> {
    private static final Logger LOG = LoggerFactory.getLogger(RechenbergSwapMutation.class);
    private Random random = new Random();

    private void swap(List<Integer> integers, int i1, int i2) {
    ...
    protected void doMutate(List<Integer> integers, int index) {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            double fitness = 0;
            try {
                fitness = new JobShopEvaluator().evaluate(new VectorSolution<Integer>(integers));
            } catch (IOException e) {
